...
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, AccountRequest> accountRequestConsumerFactory() {
        JsonDeserializer<AccountRequest> deserialiser = new JsonDeserializer<AccountRequest>(AccountRequest.class);
        deserialiser.addTrustedPackages("*");
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapAddress);
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "accountRequest");
        return new DefaultKafkaConsumerFactory<>(props, new StringDeserializer(), deserialiser);
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, AccountRequest> accountRequestConcurrentKafkaListenerContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, AccountRequest> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(accountRequestConsumerFactory());
        return factory;
    }
